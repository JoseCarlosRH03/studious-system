...
public partial class Logic
{
    ...
    public async Task<int> CreateDriver(DriverData data)
    {
        ...
        try
        {
            ...
            {
                resultFile = await SaveFail(data.Name, data.Type, data.Dataurl, data.Size);
            }
    ...
    public async Task<int> UpdateDriver(int id, DriverData data, User user)
    {
        Validation.ValidateDriverData(data);
        int? resultFile = 0;
        ...
        await Data.Atomic(async () => {
            ...
            if (driver.LicenseFileId is null && data.Dataurl.Length > 0)
            {

                resultFile = await SaveFail(data.Name, data.Type, data.Dataurl, data.Size);

            }
            else if (data.Dataurl.Length > 0)
            {
                resultFile = await SaveFail(data.Name, data.Type, data.Dataurl, data.Size, user.Id, driver.LicenseFileId);


            }

            driver.LicenseFileId = resultFile;
            await Data.Update<Driver>(driver, user.Id);
    ...
    public async Task<int> InactiveDriver(int id, User user)
    {
        var driver = await Data.GetByIdAsync<Driver>(id);
        if (driver == null)
            throw new NotFound("No existe este conductor");
